...
public class StudentRepositoryWithBagRelationshipsImpl implements StudentRepositoryWithBagRelationships {
    ...
    public Optional<Student> fetchBagRelationships(Optional<Student> student) {
        return student.map(this::fetchSkills);
    }
    ...
    public List<Student> fetchBagRelationships(List<Student> students) {
        return Optional.of(students).map(this::fetchSkills).orElse(Collections.emptyList());
    }

    Student fetchSkills(Student result) {
        return entityManager
            .createQuery("select student from Student student left join fetch student.skills where student is :student", Student.class)
            .setParameter("student", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Student> fetchSkills(List<Student> students) {
        ...
        List<Student> result = entityManager
            .createQuery(
                "select distinct student from Student student left join fetch student.skills where student in :students",
                Student.class
            )
