...
public class SkillRepositoryWithBagRelationshipsImpl implements SkillRepositoryWithBagRelationships {
    ...
    public Optional<Skill> fetchBagRelationships(Optional<Skill> skill) {
        return skill.map(this::fetchStudents);
    }
    ...
    public List<Skill> fetchBagRelationships(List<Skill> skills) {
        return Optional.of(skills).map(this::fetchStudents).orElse(Collections.emptyList());
    }

    Skill fetchStudents(Skill result) {
        return entityManager
            .createQuery("select skill from Skill skill left join fetch skill.students where skill is :skill", Skill.class)
            .setParameter("skill", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Skill> fetchStudents(List<Skill> skills) {
        ...
        List<Skill> result = entityManager
            .createQuery("select distinct skill from Skill skill left join fetch skill.students where skill in :skills", Skill.class)
